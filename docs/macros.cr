...
# Note that the node is pasted as-is. If in the previous example we pass a symbol,
...
# ```
#
# Note that `:foo` was the result of the interpolation, because that's what was passed
# to the macro. You can use the method `ASTNode#id` in these cases, where you just
# need an identifier.
#
# ### Macro calls
#
...
# ```
#
# Similar to regular code, `Nop`, `NilLiteral` and a false `BoolLiteral` are considered
# *falsey*, while everything else is considered truthy.
#
...
#
# Additional, using `*` when interpolating an `ArrayLiteral` interpolates
...
# ### Fresh variables
#
# Once macros generate code, they are parsed with a regular Crystal parser where
# local variables in the context of the macro invocations are assumed to be defined.
...
# ```
#
# This can sometimes be useful to avoid repetitive code by actually accessing
# and reading/writing local varaibles, but can also overwrite local variables
# by mistake. You can use fresh variables with `%name`:
#
# ```text
# macro dont_update_x
#   %x = 1
...
# ```
#
# Using `%x` in the above example we declare a variable whose name is guaranteed
# not to conflict with local varaibles in the current scope. Additionally, you
# can declare fresh variables related to some other AST node using
# `%var{key1, key2, ..., keyN}`. For example:
#
# ```
...
# ```
#
# In the above example three variables were declared, associated to an index, and
# then they were printed, refering to these variables with the same indices.
#
# ### Type information
#
# When a macro is invoked you can access the current scope, or type, with a special
# instance variable: `@type`. The type of this variable is `TypeNode`, which gives
# you access to type information at compile time.
#
# ### Constants
#
...
# ### Macro defs
#
# Macro defs allow you to define a method for a class hierarchy and have that
# method be evaluated at the end of the type-inference phase, as a macro,
# where type information is known, for each concrete subtype. For example:
#
# ```
# class Object
