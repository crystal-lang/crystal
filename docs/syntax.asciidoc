...
If an `if` doesn't have an `else` branch and the condition is falsey,
the `if`'s value will be `nil`:
...
# Here a will be nil (a's type is Int32 or Nil)
-------------
...
This even applies if there are ands (`&&`) in the condition:
