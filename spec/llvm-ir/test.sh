...
function test() {
  ...
  output_ll="$BUILD_DIR/test-ir.ll"
  compiler_options=()
  if [[ -n "${2:-}" ]]; then
    read -r -a compiler_options <<< "$2"
  fi
  filecheck_args=()
  if [[ -n "${3:-}" ]]; then
    filecheck_args+=(--check-prefix "$3")
  fi
  ...
  "$crystal" build --single-module --no-color --emit=llvm-ir "${compiler_options[@]}" -o "$BUILD_DIR"/test-ir "$input_cr"
  if [[ ${#filecheck_args[@]} -gt 0 ]]; then
    $FILE_CHECK "$input_cr" --input-file "$output_ll" "${filecheck_args[@]}"
  else
    $FILE_CHECK "$input_cr" --input-file "$output_ll"
  fi
