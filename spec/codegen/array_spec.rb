...
describe 'Code gen: array' do
  ...
  it "codegens array push" do
    run('require "array"; a = []; a << 1; a << 2; a[0] + a[1]').to_i.should eq(3)
  end

  it "realloc array buffer when pushing" do
    run('require "array"; a = []; j = 0; while j < 10000; a << 1; j += 1; end')
  end

  it "codegens an empty array" do
    run('require "array"; a = []; a.length').to_i.should eq(0)
  end

  it "codegens recursive array" do
    run('require "array"; a = []; a << a; a.length').to_i.should eq(1)
  end

  it "codegens array set in recursive union" do
    run('require "array"; a = [0] ; a[0] = a; a.length').to_i.should eq(1)
  end

  it "codegens method with array mutation" do
    run('require "array"; def foo(x); end; a = []; foo a; a.push(1)')
  end

  it "codegens method with array mutation and while" do
    run('require "array"; def foo(x); while false; end; end; a = []; foo a; a.push(1)')
  end
