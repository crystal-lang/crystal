...
describe OAuth::Signature do
  describe "key" do
    it "gets when token secret is empty" do
      signature = OAuth::Signature.new("consumer_key", "consumer secret", oauth_token_secret: nil)
      signature.key.should eq("consumer%20secret&")
    end

    it "gets when token secret is not empty" do
      signature = OAuth::Signature.new("consumer_key", "consumer secret", oauth_token_secret: "token secret")
  ...
  describe "base string" do
    it "computes without port in host" do
      ...
      base_string = signature.base_string request, ssl, ts, "nonce", "some+callback"
  ...
  describe "compute" do
    ...
    it "computes twitter sample" do
      ...
      signature = OAuth::Signature.new consumer_key, consumer_secret, oauth_token, oauth_token_secret
