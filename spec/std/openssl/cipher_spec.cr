...
describe OpenSSL::Cipher do
  ...
  ifdef linux
    it "encrypts/decrypts with tag" do
      cipher = "aes-256-gcm"
      c1 = OpenSSL::Cipher.new(cipher)
      c2 = OpenSSL::Cipher.new(cipher)
      key = "\0" * 32
      iv = "\0" * 12
      data = "DATA" * 5
      ciphertext = File.read(File.join(__DIR__ + "/cipher_spec.tag.ciphertext"))
      tag = File.read(File.join(__DIR__ + "/cipher_spec.tag"))
      ...
      s1.to_slice.should eq(ciphertext.to_slice)
      s1.to_slice.should eq(s2.to_slice)
      c1.tag.should eq(tag.to_slice)
      c2.tag.should eq(c1.tag)
