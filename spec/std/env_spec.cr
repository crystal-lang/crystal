...
describe "ENV" do
  ...
  it "set and gets" do
    ENV.unsafe_set("FOO", "1").should be_nil
    ENV["FOO"].should eq("1")
    ENV["FOO"]?.should eq("1")
  end
  ...
  it ".[]= (deprecated)" do
    ...
    (ENV["FOO"] = "bar").should eq("bar")
    ENV["FOO"]?.should eq("bar")

    (ENV["FOO"] = nil).should be_nil
    ENV["FOO"]?.should be_nil
  end
  ...
  it "sets to nil (same as delete)" do
    ...
    ENV.unsafe_set("FOO", nil)
    ENV["FOO"]?.should be_nil
  end

  it "sets to empty string" do
    ENV.unsafe_set("FOO_EMPTY", "")
  ...
  it "does .keys" do
    ENV.unsafe_set("FOO", "1")
    ENV.unsafe_set("BAR", "1")
    %w(FOO BAR).each { |k| ENV.keys.should contain(k) }

    ENV.unsafe_set("FOO", nil)
    ENV.unsafe_set("BAR", nil)
  ...
  it "does .values" do
    ENV.unsafe_set("FOO", "SOMEVALUE_1")
    ENV.unsafe_set("BAR", "SOMEVALUE_2")
  ...
  it "handles unicode" do
    ENV.unsafe_set("TEST_UNICODE_1", "bar\u{d7ff}\u{10000}")
    ENV.unsafe_set("TEST_UNICODE_2", "\u{1234}")
  ...
  {% if flag?(:win32) %}
    it "skips internal environment variables" do
      ...
      expect_raises(ArgumentError) { ENV.unsafe_set(key, "foo") }
      expect_raises(ArgumentError) { ENV.unsafe_set(key, nil) }
    end
  {% end %}
end
