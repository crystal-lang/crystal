...
describe Time do
  ...
  it "parse fails without time zone" do
    expect_raises(Time::Format::Error, "no default location provided") do
      Time.parse("2017-12-01 20:15:13", "%F %T", nil)
    end
    Time.parse("2017-12-01 20:15:13", "%F %T", Time::Location.local).to_s("%F %T").should eq "2017-12-01 20:15:13"
    Time.parse("2017-12-01 20:15:13 +01:00", "%F %T %:z", nil).to_s("%F %T %:z").should eq "2017-12-01 20:15:13 +01:00"
  end
  ...
  it "parses timezone" do
    ...
    {"+0000", "+00:00", "+00:00:00"}.zip(patterns) do |string, pattern|
      time = Time.parse(string, pattern, nil)
    ...
    time = Time.parse("+04:12:39", "%::z", nil)
  ...
  it "raises when time zone missing" do
    ...
    expect_raises(Time::Format::Error, "Invalid timezone") do
      Time.parse("123456+01:00", "%3N%z", nil)
    end
  end
  ...
  it do
    time = Time.parse("2014-10-31 10:11:12 Z hi", "%F %T %z hi", nil)
    time.utc?.should be_true
    time.to_utc.to_s.should eq("2014-10-31 10:11:12 UTC")
  end

  it do
    time = Time.parse("2014-10-31 10:11:12 UTC hi", "%F %T %z hi", nil)
    time.utc?.should be_true
    time.to_utc.to_s.should eq("2014-10-31 10:11:12 UTC")
  end

  it do
    time = Time.parse("2014-10-31 10:11:12 -06:00 hi", "%F %T %z hi", nil)
    time.utc?.should be_false
    time.location.fixed?.should be_true
  ...
  it do
    time = Time.parse("2014-10-31 10:11:12 +05:00 hi", "%F %T %z hi", nil)
    time.utc?.should be_false
    time.location.fixed?.should be_true
  ...
  it do
    time = Time.parse("2014-10-31 10:11:12 -06:00:00 hi", "%F %T %z hi", nil)
    time.utc?.should be_false
    time.location.fixed?.should be_true
    time.offset.should eq -6 * 3600
    time.to_utc.to_s.should eq("2014-10-31 16:11:12 UTC")
  ...
  it do
    time = Time.parse("2014-10-31 10:11:12 -060000 hi", "%F %T %z hi", nil)
    time.utc?.should be_false
    time.location.fixed?.should be_true
    time.offset.should eq -6 * 3600
    time.to_utc.to_s.should eq("2014-10-31 16:11:12 UTC")
  ...
  it "parses centiseconds" do
    time = Time.parse("2016-09-09T17:03:28.45+01:00", "%FT%T.%L%z", nil).to_utc
  ...
  it "parses milliseconds with %L" do
    time = Time.parse("2016-09-09T17:03:28.456+01:00", "%FT%T.%L%z", nil).to_utc
    time.to_s.should eq("2016-09-09 16:03:28 UTC")
  ...
  it "parses milliseconds with %3N" do
    time = Time.parse("2016-09-09T17:03:28.456+01:00", "%FT%T.%3N%z", nil).to_utc
    time.to_s.should eq("2016-09-09 16:03:28 UTC")
    time.millisecond.should eq(456)
    time.nanosecond.should eq(456000000)
  end
  ...
  it "parses microseconds with %6N" do
    time = Time.parse("2016-09-09T17:03:28.456789+01:00", "%FT%T.%6N%z", nil).to_utc
    time.to_s.should eq("2016-09-09 16:03:28 UTC")
    time.millisecond.should eq(456)
  ...
  it "parses nanoseconds" do
    time = Time.parse("2016-09-09T17:03:28.456789123+01:00", "%FT%T.%N%z", nil).to_utc
    time.to_s.should eq("2016-09-09 16:03:28 UTC")
    time.nanosecond.should eq(456789123)
  end

  it "parses nanoseconds with %9N" do
    time = Time.parse("2016-09-09T17:03:28.456789123+01:00", "%FT%T.%9N%z", nil).to_utc
    time.to_s.should eq("2016-09-09 16:03:28 UTC")
    time.nanosecond.should eq(456789123)
  end
  ...
  it "parses discarding additional decimals" do
    ...
    time = Time.parse("2016-09-09T17:03:28.456789123999999+01:00", "%FT%T.%N%z", nil)
  ...
  it "parses if some decimals are missing" do
    time = Time.parse("2016-09-09T17:03:28.45+01:00", "%FT%T.%3N%z", nil).to_utc
    time.nanosecond.should eq(450000000)

    time = Time.parse("2016-09-09T17:03:28.45678+01:00", "%FT%T.%6N%z", nil).to_utc
    time.nanosecond.should eq(456780000)

    time = Time.parse("2016-09-09T17:03:28.4567891+01:00", "%FT%T.%9N%z", nil).to_utc
