...
describe "Range" do
  ...
  describe "bsearch" do
    it "Int" do
      ary = [3, 4, 7, 9, 12]
      (0..ary.size - 1).bsearch { |i| ary[i] >= 2 }.should eq 0
      (0..ary.size - 1).bsearch { |i| ary[i] >= 4 }.should eq 1
      (0..ary.size - 1).bsearch { |i| ary[i] >= 6 }.should eq 2
      (0..ary.size - 1).bsearch { |i| ary[i] >= 8 }.should eq 3
      (0..ary.size - 1).bsearch { |i| ary[i] >= 10 }.should eq 4
      (0..ary.size - 1).bsearch { |i| ary[i] >= 100 }.should eq nil
      (0..ary.size - 1).bsearch { |i| true }.should eq 0
      (0..ary.size - 1).bsearch { |i| false }.should eq nil

      ary = [0, 100, 100, 100, 200]
      (0..ary.size - 1).bsearch { |i| ary[i] >= 100 }.should eq 1

      (0_i8..10_i8).bsearch { |x| x >= 10 }.should eq 10_i8
      (0_i8..9_i8).bsearch { |x| x >= 10 }.should eq nil
      (-10_i8..9_i8).bsearch { |x| x >= -5 }.should eq -5_i8

      (0_u8..10_u8).bsearch { |x| x >= 10 }.should eq 10_u8
      (0_u8..9_u8).bsearch { |x| x >= 10 }.should eq nil
      (0_u32..10_u32).bsearch { |x| x >= 10 }.should eq 10_u32
      (0_u32..9_u32).bsearch { |x| x >= 10 }.should eq nil

      (BigInt.new("-10")..BigInt.new("9")).bsearch { |x| x >= -5 }.should eq BigInt.new("-5")
    end

    it "Float" do
      inf = Float64::INFINITY
      (0.0..99.0).bsearch { |x| x > 0 && Math.log(x / 10) >= 0 }.not_nil!.should be_close(10.0, 0.0001)
      ...
      (inf..9).bsearch { |x| x > 0 && Math.log(x / 10) >= 0 }.should be_nil
      ...
      (-1.0..-1.0).bsearch { |x| x >= 0 }.should be_nil
      ...
      (0_f32..4_f32).bsearch { |x| x >= 5_f32 }.should be_nil
  ...
  describe "each iterator" do
    ...
    it "is empty with ... and begin > end" do
      (1..-1).each.to_a.empty?.should be_true
    end
  ...
  describe "reverse_each iterator" do
    ...
    it "is empty with ... and begin > end" do
      (1..-1).reverse_each.to_a.empty?.should be_true
    end
  ...
  describe "step iterator" do
    ...
    it "is empty with ... and begin > end" do
      (1..-1).step(1).to_a.empty?.should be_true
    end
