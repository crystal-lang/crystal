...
describe "FileUtils" do
  describe "cd" do
    ...
    it "raises" do
      expect_raises(IO::NotFoundError, "Error while changing directory: '/nope'") do
        FileUtils.cd("/nope")
      end
    end
  ...
  describe "mv" do
    ...
    it "raises an error if non correct arguments" do
      with_tempfile("mv-nonexitent") do |path|
        expect_raises(IO::NotFoundError, "Error renaming file: '#{File.join(path, "a")}' -> '#{File.join(path, "b")}'") do
  ...
  it "tests mkdir with an existing path" do
    expect_raises(IO::FileSystemError, "Unable to create directory: '#{datapath}'") do
  ...
  it "tests mkdir_p with multiple existing path" do
    ...
    with_tempfile("mkdir_p-existing") do |path|
      expect_raises(IO::FileSystemError, "Unable to create directory: '#{datapath("test_file.txt")}'") do
  ...
  it "tests rmdir with an non existing path" do
    with_tempfile("rmdir-nonexisting") do |path|
      expect_raises(IO::NotFoundError, "Unable to remove directory: '#{path}'") do
  ...
  it "tests rmdir with multiple non existing path" do
    with_tempfile("rmdir-nonexisting") do |path|
      expect_raises(IO::NotFoundError, "Unable to remove directory: '#{path}1'") do
  ...
  it "tests rmdir with a path that cannot be removed" do
    expect_raises(IO::FileSystemError, "Unable to remove directory: '#{datapath}'") do
  ...
  it "tests rm with some non existing paths" do
    with_tempfile("rm-nonexistent1", "rm-nonexistent2") do |path1, path2|
      ...
      expect_raises(IO::NotFoundError, "Error deleting file: '#{path2}'") do
  ...
  describe "ln" do
    ...
    it "fails with a nonexistent source" do
      ...
      ex = expect_raises(IO::NotFoundError, "Error creating link: '#{path1}' -> '#{path2}'") do
        FileUtils.ln(path1, path2)
      end
    end
    ...
    it "fails with an extant destination" do
      ...
      begin
        ...
        expect_raises(IO::FileSystemError, "Error creating link: '#{path1}' -> '#{path2}'") do
          FileUtils.ln(path1, path2)
        end
      ensure
        FileUtils.rm_rf([path1, path2])
  ...
  describe "ln_s" do
    ...
    it "works with a nonexistent source" do
      ...
      begin
        ...
        expect_raises(IO::NotFoundError, "Error resolving real path: '#{path2}'") do
    ...
    it "fails with an extant destination" do
      ...
      begin
        ...
        expect_raises(IO::FileSystemError, "Error creating symlink: '#{path1}' -> '#{path2}'") do
          FileUtils.ln_s(path1, path2)
        end
      ensure
        FileUtils.rm_rf([path1, path2])
