...
describe "Code gen: c union" do
  it "codegens union property default value" do
    run("#{CodeGenUnionString}; bar = Pointer(Foo::Bar).malloc(1_u64); bar->x").to_i.should eq(0)
  end

  it "codegens union property default value 2" do
    run("#{CodeGenUnionString}; bar = Pointer(Foo::Bar).malloc(1_u64); bar->z").to_f32.should eq(0)
  end
  ...
  it "codegens union property setter 1 via pointer" do
    run("#{CodeGenUnionString}; bar = Pointer(Foo::Bar).malloc(1_u64); bar->x = 42; bar->x").to_i.should eq(42)
  end

  it "codegens union property setter 2 via pointer" do
    run("#{CodeGenUnionString}; bar = Pointer(Foo::Bar).malloc(1_u64); bar->z = 42.0_f32; bar->z").to_f32.should eq(42.0)
  end
