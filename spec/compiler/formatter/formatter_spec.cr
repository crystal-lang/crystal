...
describe Crystal::Formatter do
  ...
  assert_format "def foo\n1\nend", "def foo\n  1\nend"
  assert_format "def   foo  =  1", "def foo = 1"
  assert_format "def foo = begin\n1\n2\nend", "def foo = begin\n  1\n  2\nend"
  assert_format "def foo = if true\n1\nelse\n2\nend", "def foo = if true\n  1\nelse\n  2\nend"
  assert_format "def foo = 1 # comment"
  assert_format "def any? = ! empty?", "def any? = !empty?"
  assert_format "def default_value = nil"
  assert_format "def initialize(s : Int32) = new(s.to_s)"
  assert_format %(def to_s(io) = io << "nil")
  assert_format "def name = @name\ndef name=(@name)\nend", "def name = @name\n\ndef name=(@name)\nend"
  assert_format "def foo\n\n1\n\nend", "def foo\n  1\nend"
  assert_format "def foo()\n1\nend", "def foo\n  1\nend"
  assert_format "def foo   (   )   \n1\nend", "def foo\n  1\nend"
  ...
  assert_format "def   foo (  x , y )  =  x + y", "def foo(x, y) = x + y"
  assert_format "def   foo (  x , y , )  \n  end", "def foo(x, y)\nend"
  assert_format "def   foo (  x , y ,\n)  \n  end", "def foo(x, y)\nend"
  assert_format "def   foo (  x ,\n y )  \n  end", "def foo(x,\n        y)\nend"
  assert_format "def   foo (\nx ,\n y )  \n  end", "def foo(\n  x,\n  y,\n)\nend"
  ...
  assert_format "private def   foo  =  1", "private def foo = 1"
  assert_format "protected def   foo  =  1", "protected def foo = 1"
  assert_format "def foo=(x)\nend"
  assert_format "def +(x)\nend"
  assert_format "def   foo  :  Int32 \n  end", "def foo : Int32\nend"
  assert_format "def   foo ( x )  :  Int32 \n  end", "def foo(x) : Int32\nend"
  ...
  assert_format "def foo(x : X)  forall   X ,   Y  =  x", "def foo(x : X) forall X, Y = x"
  assert_format "private def check_needs_resize  =  double_capacity if @size == @capacity", "private def check_needs_resize = double_capacity if @size == @capacity"
  assert_format "private def double_capacity    = resize_to_capacity(@capacity == 0 ? 3 : (@capacity * 2))", "private def double_capacity = resize_to_capacity(@capacity == 0 ? 3 : (@capacity * 2))"
  assert_format "def foo(x)\n  self // x\nend"
  assert_format "def foo(x)\n  case self // x\n  when 2\n    3\n  end\nend"
  assert_format "def foo(x)\n  case 1\n  when self // 2\n    3\n  end\nend"
  assert_format "def foo(x)\n  case //\n  when //\n    3\n  end\nend"
