...
private def describe_spec(file, smart = false, render = false)
...
private def assert_section(file, section, examples, smart)
  describe section do
...
private def assert_exapmle(file, section, index, example, smart)
  ...
  it "- #{index}\n#{show_space(markdown)}", file, line do
    output = Crystal::Doc::Markdown.to_html(markdown, options)
    output.should eq(html), file, line
  end
end

private def extract_spec_tests(file)
  data = [] of String
  delimiter = "`" * 32
