...
describe 'Global inference' do
  ...
  it "infers type of global assign with union" do
    ...
    nodes[0].target.type.should eq(mod.union_of(mod.int, mod.double))
    nodes[1].target.type.should eq(mod.union_of(mod.int, mod.double))
  end

  it "infers type of global reference" do
    assert_type("$foo = 1; def foo; $foo = 2.5; end; foo; $foo") { union_of(int, double) }
  end
end
