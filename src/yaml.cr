...
# File.open("foo.yml", :write, :create, :truncate) { |f| YAML.dump({hello: "world"}, f) } # writes it to the file
# # or:
# yaml = {hello: "world"}.to_yaml                                                      # => "---\nhello: world\n"
# File.open("foo.yml", :write, :create, :truncate) { |f| {hello: "world"}.to_yaml(f) } # writes it to the file
# ```
module YAML
