...
class File < IO::FileDescriptor
  ...
  # The `tempfile` method is for managing temporary files.
  #
  # ```
  ...
  # ```
  #
  # Files created from this method are stored in a directory that handles
  # temporary files.
  #
  # ```
  ...
  #
  # ```
  # File.tempfile("foo", ".png").path # => "/tmp/foo.ulBCPS.png"
  # ```
  #
  # *encoding* and *invalid* are passed to `IO#set_encoding`.
  def self.tempfile(name, extension = nil, encoding = nil, invalid = nil)
    fileno, path = Crystal::System::File.mktemp(name, extension)
    File.new(path, fileno, blocking: true, encoding: encoding, invalid: invalid)
  end
  ...
  # ```
  def self.tempfile(filename, extension = nil)
    tempfile = File.tempfile(filename, extension)
    begin
      yield tempfile
    ensure
      tempfile.close
