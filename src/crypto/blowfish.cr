class Crypto::Blowfish
  ...
  def initialize(key)
    if key.length < 1 || key.length > 56
      raise "Invalid key size: the key must be 1-56 bytes."
    end

    @key = key
    @p = [] of Int64|Int32 # Int32 is needed because crystal returns Int32 by default
    @s = [] of Array(Int64)
  ...
  def expand_key(key = @key)
    pos = Pointer(Int32).malloc(1)
    0.upto(17) do |i|
      @p[i] = (@p[i] ^ next_word(key, pos)) % ULONG
    end
  ...
  def salted_expand_key(salt, key)
    ...
    0.step(17, 2) do |i|
      l, r = encrypt_pair(l ^ next_word(salt, pos), r ^ next_word(salt, pos))
      @p[i] = l
      @p[i+1] = r
    end
