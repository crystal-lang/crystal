...
class Hash(K, V)
  ...
  protected def find_entry_with_index(key : K) : {Entry(K, V), Int32}?
  ...
  private def find_entry_with_index_linear_scan(key : K) : {Entry(K, V), Int32}?
  ...
  def ==(other : Hash(K2, V2)) : Bool forall K2, V2
    {% if K <= K2 %}
