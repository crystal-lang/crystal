module Crystal
  module Annotatable
    ...
    property applied_annotations : Hash(AnnotationType, Array(Annotation))?

    # Adds an annotation with the given type and value
    def add_annotation(annotation_type : AnnotationType, value : Annotation)
      annotations = @applied_annotations ||= {} of AnnotationType => Array(Annotation)
    ...
    def annotation(annotation_type : AnnotationType) : Annotation?
      @applied_annotations.try &.[annotation_type]?.try &.last?
    end
