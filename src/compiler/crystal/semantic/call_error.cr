...
class Crystal::Call
  def raise_matches_not_found(owner, def_name, arg_types, named_args_types, matches = nil, with_autocast = false)
    obj = @obj
    ...
    if with_autocast == false && (args.any?(&.supports_autocast?) ||
       named_args.try &.any? &.value.supports_autocast?)
