module Crystal
  class ASTNode
    ...
    def type
      @type || unreachable! "`#{self}` at #{self.location} has no type"
    end
    ...
    def set_enclosing_call(enclosing_call)
      unreachable! "already had enclosing call" if @enclosing_call
            ...
            if node.is_a?(Path) && (target_const = node.target_const)
              ...
              if value.is_a?(NumberLiteral)
                ...
                if visitor
                  ...
                  numeric_type = node_type.program.int?(numeric_value) || unreachable! "expected integer type, not #{numeric_value.class}"
