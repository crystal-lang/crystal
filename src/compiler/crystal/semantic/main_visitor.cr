...
module Crystal
  ...
  class MainVisitor < SemanticVisitor
    ...
    def visit(node : Case)
      expansion = @program.literal_expander.expand node
      assigns, last = if expansion.is_a?(Expressions)
                        last_if = expansion.expressions.pop.as(If)
                        expansion.expressions.each &.accept self
                        {expansion.expressions, last_if}
                      else
                        {nil, expansion.as(If)}
                      end

      if check_exhaustiveness(node)
        last_last = last
        while (last_last_else = last_last.else).is_a?(If)
      ...
      expansion = if assigns && !assigns.empty?
                    assigns << last
                    Expressions.from assigns
                  else
                    last
                  end
      expansion.at(node)
      expansion.bind_to last
      node.expanded = expansion
    ...
    def check_exhaustiveness(node)
      # If there is 'else' block, then it has exhaustiveness.
      return false if node.else

      # Can't check exhaustiveness without cond clause expression.
      return false unless case_cond = node.cond

      if case_cond.is_a?(TupleLiteral)
        check_exhaustiveness_tuple(case_cond, node)
      else
        check_exhaustiveness_simple(case_cond, node)
      end
    end

    alias Pattern = Nil | Bool | Type

    def check_exhaustiveness_tuple(case_tuple, node)
      ...
      node.whens.each &.conds.each do |when_cond|
        case when_cond
        when Underscore
          return true
        when TupleLiteral
    ...
    def check_exhaustiveness_simple(case_cond, node)
    ...
    def calculate_patterns(case_cond)
      case_cond.accept self if case_cond.is_a?(Var) || case_cond.is_a?(InstanceVar)
      ...
      when EnumType
        ...
        return false if case_cond_type.flags?
    ...
    def check_exhaustiveness_step(patterns, when_cond)
      case when_cond
      when Path
        type_or_const = (@path_lookup || @scope || @current_type).lookup_type_var?(when_cond, free_vars: free_vars)
      ...
      when Generic
        if type = (@path_lookup || @scope || @current_type).lookup_type?(when_cond, free_vars: free_vars)
          check_exhaustiveness_step_type(patterns, type) { |pattern| yield pattern }
        end
      when IsA
        if when_cond.obj.is_a?(ImplicitObj)
          if type = (@path_lookup || @scope || @current_type).lookup_type?(when_cond.const, free_vars: free_vars)
            check_exhaustiveness_step_type(patterns, type) { |pattern| yield pattern }
          end
        end
      when Call
