    ...
    def expand(node : Select)
      ...
      node.whens.each_with_index do |a_when, index|
        ...
        when Assign
          ...
          node.unreachable! "expected select when expression to be Assign or Call, not #{condition}"
        end
      end
      ...
      if node_else = node.else
        ...
        case_else = Call.new(nil, "unreachable!", args: [StringLiteral.new("invalid select index")] of ASTNode, global: true).at(node)
      end
