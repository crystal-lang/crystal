module Crystal::Doc::Markdown::Parser
  # :nodoc:
  class Inline
    include Parser
    ...
    private def entity(node : Node)
      if char_at?(@pos) == '&'
        ...
        decoded_text = Markdown::HTMLEntities.decode_entity text
    ...
    def decode_uri(text : String)
      ...
      if decoded.includes?('&') && decoded.includes?(';')
        decoded = decoded.gsub(/^&(\w+);$/) { |chars| Markdown::HTMLEntities.decode_entities(chars) }
      end
      decoded
    end
