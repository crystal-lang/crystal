...
# :nodoc:
module Crystal::Doc::Markdown::HTMLEntities
  def self.decode_entities(source : String)
    Decoder.decode(source)
  end

  def self.decode_entity(source : String)
    Decoder.decode_entity(source)
  end

  def self.encode_entities(source)
  ...
  module Decoder
    ...
    def self.decode_entity(chars)
      if chars[0] == '#'
        ...
        if resolved_entity = Markdown::HTMLEntities::ENTITIES_MAPPINGS[entities_key]?
    ...
    def self.decode_codepoint(codepoint)
      ...
      if decoded = Markdown::HTMLEntities::DECODE_MAPPINGS[codepoint]?
        codepoint = decoded
      end
