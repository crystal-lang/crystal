...
module Crystal::Macros
  ...
  abstract class ASTNode
    ...
    # If *comment* is `true`, each line is prefixed with a `#` character to allow the output to be used within another node's documentation comment.
    #
    # The primary use case for this method is to be paired with the `@caller` macro instance variable in order to merge the documentation comments
    # on a node generated by a macro, and the comments on the macro call itself.
    #
    # ```
    # macro gen_method(name)
    #   # {{ @caller.docs(comment: true).id }}
    #   #
    ...
    # ```
    #
    # WARNING: This method returns an empty string when executed outside of the `crystal docs` command.
    def docs(*, comment : BoolLiteral) : StringLiteral
    end
