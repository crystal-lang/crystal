module Crystal
  class CrystalLLVMBuilder
    ...
    def current_debug_location=(debug_location)
      @builder.current_debug_location = debug_location
    end
    ...
    {% for name in %w(add add_handler alloca and ashr atomicrmw bit_cast build_catch_ret call
                     catch_pad catch_switch cmpxchg cond exact_sdiv extract_value fadd fcmp fdiv
                     fence fmul fp2si fp2ui fpext fptrunc fsub global_string_pointer icmp
                     inbounds_gep int2ptr invoke landing_pad load lshr mul not or phi ptr2int sdiv
                     select sext shl si2fp srem store store_volatile sub switch trunc udiv ui2fp
