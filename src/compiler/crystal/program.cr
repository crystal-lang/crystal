...
module Crystal
  ...
  class Program < NonGenericModuleType
    ...
    def initialize
      ...
      types["Constructor"] = @constructor_annotation = AnnotationType.new self, self, "Constructor"
    ...
    {% for name in %w(object no_return value number reference void nil bool char int int8 int16 int32 int64 int128
                     ...
                     flags_annotation link_annotation extern_annotation deprecated_annotation experimental_annotation constructor_annotation) %}
      def {{name.id}}
        @{{name.id}}.not_nil!
      end
    {% end %}
