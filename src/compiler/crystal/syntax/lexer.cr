...
module Crystal
  class Lexer
    ...
    getter macro_expansion_pragmas : Hash(Int32, Array(LocPragma))? = nil

    # `macro_expansion_pragmas` ordered by position, and the index of the next
    # pragma to apply. `#lookahead` can restore an index after a failed parse.
    @ordered_macro_expansion_pragmas : Array({Int32, Array(LocPragma)})? = nil
    @macro_expansion_pragmas_index = 0

    def macro_expansion_pragmas=(pragmas : Hash(Int32, Array(LocPragma))?)
      @macro_expansion_pragmas = pragmas
      @ordered_macro_expansion_pragmas = pragmas.try &.to_a.sort_by! &.[0]
      @macro_expansion_pragmas_index = 0
      pragmas
    end
    ...
    def next_token
      ...
      if me_pragmas = @ordered_macro_expansion_pragmas
        ...
        # incorrect location.
        while (index = @macro_expansion_pragmas_index) < me_pragmas.size &&
              me_pragmas[index][0] <= start
          me_pragmas[index][1].each &.run_pragma self
          @macro_expansion_pragmas_index = index + 1
        end
      end
    ...
    def lookahead(preserve_token_on_fail = false, &)
      ...
      # A failed lookahead leaves the parser to lex the same text again, so it
      # has to see the same location state and the same pragmas as before.
      old_filename = @filename
      old_stacked, old_stacked_filename = @stacked, @stacked_filename
      old_stacked_line, old_stacked_column = @stacked_line_number, @stacked_column_number
      old_pragmas_index = @macro_expansion_pragmas_index
      ...
      unless result
        ...
        @stacked, @stacked_filename = old_stacked, old_stacked_filename
        @stacked_line_number, @stacked_column_number = old_stacked_line, old_stacked_column
        @macro_expansion_pragmas_index = old_pragmas_index
