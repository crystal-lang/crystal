  ...
  class Parser < Lexer
    ...
    def parse_expression_suffix(atomic, location)
      while true
        ...
        when .op_bracketr?, .op_comma?, .op_semicolon?, .op_percent_curlyr?, .newline?, .eof?
          break
        else
          if end_token?
            break
    ...
    AtomicWithMethodCheck = [
      ...
      :OP_LT_EQ_GT, :OP_EQ_EQ_EQ, :OP_SQUAREL_SQUARER, :OP_SQUAREL_SQUARER_EQ, :OP_SQUAREL_SQUARER_QUESTION,
      :OP_SQUAREL, :OP_AMP_PLUS, :OP_AMP_MINUS, :OP_AMP_STAR, :OP_AMP_STAR_STAR,
    ] of Token::Kind
    ...
    def parse_atomic_method_suffix_special(call, location)
      case @token.type
      when .op_period?, .op_squarel?, .op_squarel_squarer?
    ...
    def parse_atomic_without_location
      ...
      when .op_curlyl_curlyl?
        parse_percent_macro_expression
      when .op_curlyl_percent?
    ...
    def parse_parenthesized_expression
      ...
      unexpected_token if @token.type.op_bracketl?
    ...
    def parse_fun_literal
      ...
      when .op_bracketl?, .op_curlyl?, .op_colon?
        # do nothing
      else
    ...
    def parse_fun_pointer
      ...
      if @token.type.op_bracketl?
        next_token_skip_space
        types = parse_union_types(:OP_BRACKETR)
        check :OP_BRACKETR
        next_token_skip_space
      else
        types = [] of ASTNode
    ...
    def parse_macro_if(start_location, macro_state, check_end = true, is_unless = false)
      ...
      if !@token.type.op_percent_curlyr? && check_end
        if is_unless
    ...
    DefOrMacroCheck2 = [
      ...
      :OP_SQUAREL_SQUARER, :OP_SQUAREL_SQUARER_EQ, :OP_SQUAREL_SQUARER_QUESTION, :OP_LT_EQ_GT,
    ...
    def parse_arg(args, extra_assigns, parentheses, found_default_value, found_splat, found_double_splat, allow_restrictions)
      ...
      if splat && (@token.type.op_comma? || @token.type.op_bracketr?)
        arg_name = ""
        uses_arg = false
        allow_restrictions = false
      else
    ...
    def parse_block_arg(extra_assigns)
      ...
      if @token.type.op_bracketr? || @token.type.newline? || @token.type.op_colon?
        arg_name = ""
      else
        ...
        while true
          ...
          when .op_bracketl?
            ...
            while true
              ...
              when .op_comma?
                next_token_skip_space_or_newline
                break if @token.type.op_bracketr?
              when .op_bracketr?
                break
              else
    ...
    def parse_call_args_space_consumed(check_plus_and_minus = true, allow_curly = false, control = false, end_token : Token::Kind = :OP_BRACKETR,
      ...
      when .char?, .string?, .delimiter_start?, .string_array_start?, .symbol_array_start?,
           ...
           .op_dollar_tilde?, .op_dollar_question?, .global_match_data_index?, .op_bracketl?,
           .op_bang?, .op_squarel?, .op_squarel_squarer?, .op_tilde?, .op_minus_gt?,
           .op_curlyl_curlyl?, .magic?, .underscore?
        # Nothing
    ...
    def parse_call_args_named_args(location, args, first_name, allow_newline)
      ...
      check :OP_BRACKETR if allow_newline
      end_location = token_end_location
    ...
    def parse_named_args(location, first_name = nil, allow_newline = false)
      ...
      while true
        ...
        if @token.type.op_comma?
          next_token_skip_space_or_newline
          if @token.type.op_bracketr? || @token.type.op_amp? || @token.type.op_squarer?
            break
          end
    ...
    def parse_atomic_type
      ...
      when .op_curlyl?
        ...
        if named_tuple_start? || @token.type.delimiter_start?
          type = make_named_tuple_type parse_named_type_args(:OP_CURLYR)
        else
          type = make_tuple_type parse_union_types(:OP_CURLYR, allow_splats: true)
        end
        check :OP_CURLYR
        next_token_skip_space
        type
      ...
      when .op_bracketl?
        ...
        if @token.type.op_bracketr?
          ...
          while @token.type.op_comma?
            next_token_skip_space_or_newline
            break if @token.type.op_bracketr? # allow trailing comma
            input_types << parse_type_splat { parse_union_type }
          end
    ...
    def parse_type_args(name)
      return name unless @token.type.op_bracketl?

      next_token_skip_space_or_newline
      args = [] of ASTNode
      if named_tuple_start? || string_literal_start?
        named_args = parse_named_type_args(:OP_BRACKETR)
      else
    ...
    def type_start?
      while @token.type.op_bracketl? || @token.type.op_curlyl?
        next_token_skip_space_or_newline
      end
    ...
    def delimiter_or_type_suffix?
      ...
      when .op_minus_gt?, .op_bar?, .op_comma?, .op_eq_gt?, .newline?, .eof?,
           .op_eq?, .op_semicolon?, .op_bracketl?, .op_bracketr?, .op_squarel?, .op_squarer?
        # -> | , => \n EOF = ; ( ) [ ]
        true
      else
        false
    ...
    def end_token?
      case @token.type
      when .op_curlyr?, .op_squarer?, .op_percent_curlyr?, .eof?
        true
      when .ident?
        case @token.value
