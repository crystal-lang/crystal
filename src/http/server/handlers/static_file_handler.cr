...
class HTTP::StaticFileHandler
  ...
  def call(context)
    unless {"GET", "HEAD"}.includes?(context.request.method)
      if @fallthrough
        ...
        context.response.status_code = HTTP::Status::METHOD_NOT_ALLOWED
    ...
    if @directory_listing && is_dir
      ...
      if cache_request?(context, last_modified)
        context.response.status_code = HTTP::Status::NOT_MODIFIED
        return
      end
  ...
  private def redirect_to(context, url)
    context.response.status_code = HTTP::Status::FOUND
