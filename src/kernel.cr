...
module AtExitHandlers
  ...
  def self.add(handler)
    handlers = @@handlers ||= [] of Exception? ->
    handlers << handler
  end
  ...
  def self.run(error)
    ...
    begin
      @@handlers.try &.reverse_each &.call error
    rescue handler_ex
...
def exit(status = 0)
  error = status == 0 ? nil : SystemExit.new(status)

  AtExitHandlers.run error
  STDOUT.flush
  STDERR.flush
