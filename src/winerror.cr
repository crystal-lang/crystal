...
class WinError < Errno
  ...
  def winerror_to_errno(winerror)
    case winerror
    when ERROR_FILE_NOT_FOUND            then Errno::ENOENT.value
    when ERROR_PATH_NOT_FOUND            then Errno::ENOENT.value
    when ERROR_TOO_MANY_OPEN_FILES       then Errno::EMFILE.value
    when ERROR_ACCESS_DENIED             then Errno::EACCES.value
    when ERROR_INVALID_HANDLE            then Errno::EBADF.value
    when ERROR_ARENA_TRASHED             then Errno::ENOMEM.value
    when ERROR_NOT_ENOUGH_MEMORY         then Errno::ENOMEM.value
    when ERROR_INVALID_BLOCK             then Errno::ENOMEM.value
    when ERROR_BAD_ENVIRONMENT           then Errno::E2BIG.value
    when ERROR_BAD_FORMAT                then Errno::ENOEXEC.value
    when ERROR_INVALID_DRIVE             then Errno::ENOENT.value
    when ERROR_CURRENT_DIRECTORY         then Errno::EACCES.value
    when ERROR_NOT_SAME_DEVICE           then Errno::EXDEV.value
    when ERROR_NO_MORE_FILES             then Errno::ENOENT.value
    when ERROR_WRITE_PROTECT             then Errno::EACCES.value
    when ERROR_BAD_UNIT                  then Errno::EACCES.value
    when ERROR_NOT_READY                 then Errno::EACCES.value
    when ERROR_BAD_COMMAND               then Errno::EACCES.value
    when ERROR_CRC                       then Errno::EACCES.value
    when ERROR_BAD_LENGTH                then Errno::EACCES.value
    when ERROR_SEEK                      then Errno::EACCES.value
    when ERROR_NOT_DOS_DISK              then Errno::EACCES.value
    when ERROR_SECTOR_NOT_FOUND          then Errno::EACCES.value
    when ERROR_OUT_OF_PAPER              then Errno::EACCES.value
    when ERROR_WRITE_FAULT               then Errno::EACCES.value
    when ERROR_READ_FAULT                then Errno::EACCES.value
    when ERROR_GEN_FAILURE               then Errno::EACCES.value
    when ERROR_SHARING_VIOLATION         then Errno::EACCES.value
    when ERROR_LOCK_VIOLATION            then Errno::EACCES.value
    when ERROR_WRONG_DISK                then Errno::EACCES.value
    when ERROR_SHARING_BUFFER_EXCEEDED   then Errno::EACCES.value
    when ERROR_BAD_NETPATH               then Errno::ENOENT.value
    when ERROR_NETWORK_ACCESS_DENIED     then Errno::EACCES.value
    when ERROR_BAD_NET_NAME              then Errno::ENOENT.value
    when ERROR_FILE_EXISTS               then Errno::EEXIST.value
    when ERROR_CANNOT_MAKE               then Errno::EACCES.value
    when ERROR_FAIL_I24                  then Errno::EACCES.value
    when ERROR_NO_PROC_SLOTS             then Errno::EAGAIN.value
    when ERROR_DRIVE_LOCKED              then Errno::EACCES.value
    when ERROR_BROKEN_PIPE               then Errno::EPIPE.value
    when ERROR_DISK_FULL                 then Errno::ENOSPC.value
    when ERROR_INVALID_TARGET_HANDLE     then Errno::EBADF.value
    when ERROR_WAIT_NO_CHILDREN          then Errno::ECHILD.value
    when ERROR_CHILD_NOT_COMPLETE        then Errno::ECHILD.value
    when ERROR_DIRECT_ACCESS_HANDLE      then Errno::EBADF.value
    when ERROR_SEEK_ON_DEVICE            then Errno::EACCES.value
    when ERROR_DIR_NOT_EMPTY             then Errno::ENOTEMPTY.value
    when ERROR_NOT_LOCKED                then Errno::EACCES.value
    when ERROR_BAD_PATHNAME              then Errno::ENOENT.value
    when ERROR_MAX_THRDS_REACHED         then Errno::EAGAIN.value
    when ERROR_LOCK_FAILED               then Errno::EACCES.value
    when ERROR_ALREADY_EXISTS            then Errno::EEXIST.value
    when ERROR_INVALID_STARTING_CODESEG  then Errno::ENOEXEC.value
    when ERROR_INVALID_STACKSEG          then Errno::ENOEXEC.value
    when ERROR_INVALID_MODULETYPE        then Errno::ENOEXEC.value
    when ERROR_INVALID_EXE_SIGNATURE     then Errno::ENOEXEC.value
    when ERROR_EXE_MARKED_INVALID        then Errno::ENOEXEC.value
    when ERROR_BAD_EXE_FORMAT            then Errno::ENOEXEC.value
    when ERROR_ITERATED_DATA_EXCEEDS_64k then Errno::ENOEXEC.value
    when ERROR_INVALID_MINALLOCSIZE      then Errno::ENOEXEC.value
    when ERROR_DYNLINK_FROM_INVALID_RING then Errno::ENOEXEC.value
    when ERROR_IOPL_NOT_ENABLED          then Errno::ENOEXEC.value
    when ERROR_INVALID_SEGDPL            then Errno::ENOEXEC.value
    when ERROR_AUTODATASEG_EXCEEDS_64k   then Errno::ENOEXEC.value
    when ERROR_RING2SEG_MUST_BE_MOVABLE  then Errno::ENOEXEC.value
    when ERROR_RELOC_CHAIN_XEEDS_SEGLIM  then Errno::ENOEXEC.value
    when ERROR_INFLOOP_IN_RELOC_CHAIN    then Errno::ENOEXEC.value
    when ERROR_FILENAME_EXCED_RANGE      then Errno::ENOENT.value
    when ERROR_NESTING_NOT_ALLOWED       then Errno::EAGAIN.value
    when ERROR_NO_DATA                   then Errno::EPIPE.value
    when ERROR_DIRECTORY                 then Errno::ENOTDIR.value
    when ERROR_NOT_ENOUGH_QUOTA          then Errno::ENOMEM.value
    else                                      Errno::EINVAL.value
    end
  end
