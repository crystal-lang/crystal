...
class UDPSocket < IPSocket
  ...
  def bind(host, port, dns_timeout = nil)
    Addrinfo.resolve(host, port, family: @family, type: Type::DGRAM, protocol: Protocol::UDP, timeout: dns_timeout) do |addrinfo|
  ...
  # Sends a text message to the previously connected remote server.
  def send(message : String)
    send(message.to_slice)
  end

  # Sends a binary message to the previously connected remote server.
  ...
  # Sends a text message to the specified remote server.
  def send(message : String, addr : IPAddress)
    send(message.to_slice, addr)
  end

  # Sends a binary message to the specified remote server.
