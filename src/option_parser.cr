...
class OptionParser
  ...
  def self.parse(args = ARGV, *, gnu_optional_args : Bool = false, gnu_strict_bundling : Bool = false, &) : self
    parser = OptionParser.new(gnu_optional_args: gnu_optional_args, gnu_strict_bundling: gnu_strict_bundling)
    yield parser
    parser.parse(args)
    parser
  end
  ...
  #
  # Refer to `#gnu_optional_args?` for the behaviour of the named parameter.
  def initialize(*, @gnu_optional_args : Bool = false, @gnu_strict_bundling : Bool = false)
    @flags = [] of String
  ...
  #
  # Refer to `#gnu_optional_args?` for the behaviour of the named parameter.
  def self.new(*, gnu_optional_args : Bool = false, gnu_strict_bundling : Bool = false, &)
    new(gnu_optional_args: gnu_optional_args, gnu_strict_bundling: gnu_strict_bundling).tap { |parser| yield parser }
  end
  ...
  # Returns whether strict GNU-style short option bundling is enabled.
  #
  ...
  # removed = false
  # forced = false
  # OptionParser.parse(%w(-rf), gnu_strict_bundling: true) do |parser|
  ...
  # ```
  #
  # Without `gnu_strict_bundling: true`, a token like `-rf` is interpreted as
  # the flag `-r` with an inline value `f`.
  property? gnu_strict_bundling : Bool
  ...
  def parse(args = ARGV) : Nil
    with_preserved_state do
      ...
      while arg_index < args.size
        ...
        if gnu_strict_bundling? && bundled_short_arg?(arg)
