...
class Deflate::Writer < IO
  # If `#sync_close?` is `true`, closing this IO will close the underlying IO.
  property? sync_close : Bool

  # Creates an instance of Flate::Writer. `close` must be invoked after all data
  # has written.
  def initialize(@output : IO, level : Int32 = Deflate::DEFAULT_COMPRESSION,
                 strategy : Deflate::Strategy = Deflate::Strategy::DEFAULT,
    ...
    raise Deflate::Error.new(ret, @stream) unless ret.ok?
  end

  # Creates a new writer for the given *io*, yields it to the given block,
  # and closes it at its end.
  def self.open(io : IO, level : Int32 = Deflate::DEFAULT_COMPRESSION,
                strategy : Deflate::Strategy = Deflate::Strategy::DEFAULT,
                sync_close : Bool = false, dict : Bytes? = nil)
