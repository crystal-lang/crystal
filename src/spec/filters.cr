...
module Spec
  module Item
    protected def matches_pattern?(pattern : Regex) : Bool
      !!(@description =~ pattern)
    end

    protected def matches_line?(line : Int32) : Bool
      @line == line || @line <= line <= @end_line
    end

    protected def matches_locations?(locations : Hash(String, Array(Int32))) : Bool
  ...
  class RootContext
    protected def filter_by_pattern(pattern : Regex)
      children.select!(&.filter_by_pattern(pattern))
    end

    protected def filter_by_line(line : Int32)
      children.select!(&.filter_by_line(line))
    end

    protected def filter_by_locations(locations : Hash(String, Array(Int32)))
      children.select!(&.filter_by_locations(locations))
    end

    protected def filter_by_focus
      children.select!(&.filter_by_focus)
    end

    protected def filter_by_split(split_filter : SplitFilter)
  ...
  class ExampleGroup
    ...
    protected def filter_by_pattern(pattern : Regex) : Bool
    ...
    protected def filter_by_line(line : Int32) : Bool
    ...
    protected def filter_by_locations(locations : Hash(String, Array(Int32))) : Bool
      # If any children matches then we match too, but we filter children
    ...
    # `false` otherwise.
    protected def filter_by_focus : Bool
    ...
    protected def filter_by_split(split_filter : SplitFilter) : Bool
      children.select!(&.filter_by_split(split_filter))
      !children.empty?
    end
  end
