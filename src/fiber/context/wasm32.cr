...
#
# The first 1024 bytes of memory are unused (but they are usable, including the address 0!).
# Follows the main stack with the mutable global `__stack_pointer` pointing at the stack top. It
# is moved by the program during execution. Then follows the read-only data and the mutable data.
# Note that WebAssembly doesn't have read only memory, so it is all mutable in practice. The
# special symbols `__data_start` and `__data_end` are generated by LLVM to mark this region.
...
#
# For our implementation of stack switching, this is the memory layout on every stack:
#
# +--------+---------------------------+------------------------+
