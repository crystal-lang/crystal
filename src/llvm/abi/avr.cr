...
class LLVM::ABI::AVR < LLVM::ABI
  ...
  # Follows GCC AVR, while Clang (and Rust) are incompatible (despite LLVM
  # itself being compliant apparently).
  #
  ...
  private def compute_return_type(rty, ret_def, context)
    if !ret_def
      ...
      # TODO: report the next pw2ceil pad of structs (<= @rsize)? or the LLVM
      #       AVR backend handles it?
      ArgType.direct(rty)
    end
  end
  ...
  private def compute_arg_types(atys, context)
    ...
    atys.map do |aty|
      ...
      if bytes == 0 || rn < @rmin
        ...
        # TODO: report the odd to next even number pad here? or the LLVM AVR
        #       backend handles it?
        ArgType.direct(aty)
      end
    end
