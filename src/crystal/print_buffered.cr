module Crystal
  ...
  # overall size is larger than PIPE_BUF, buf should at least write 512 bytes
  # atomically.
  ...
  # UIdentical to `#print_buffered` but eventually calls
  # `System.print_error(bytes)` to write to stderr without going through the
  # event loop.
  ...
  def self.buffered_message(message : String, *args, exception = nil, backtrace = nil)
