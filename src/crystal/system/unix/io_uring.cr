...
class Crystal::System::IoUring
  ...
  def self.supported? : Bool
    ...
    begin
      ...
      ret = Crystal::System::Syscall.io_uring_register(fd, LibC::IORING_REGISTER_PROBE, probe.as(Void*), LibC::IORING_OP_LAST)
  ...
  # OPTIMIZE: leverage IORING_SETUP_NO_SQARRAY when available (linux 6.6+)

  def initialize(sq_entries : UInt32, *, cq_entries : UInt32? = nil, sq_poll : Bool = false, wq : IoUring? = nil)
    # setup the ring
    params = LibC::IoUringParams.new
    ...
    @fd = Crystal::System::Syscall.io_uring_setup(sq_entries, pointerof(params))
    raise RuntimeError.from_os_error("io_uring_setup", Errno.new(-@fd)) if @fd < 0

    @flags = params.flags

    # determine the size of ring buffers
    @sqes_size = params.sq_entries * sizeof(LibC::IoUringSqe)
    @sq_size = params.@sq_off.array + params.sq_entries * sizeof(UInt32)
    ...
    # map the SQ array (ring indirection to SQE)
    ...
    if (params.features & LibC::IORING_FEAT_SINGLE_MMAP) == LibC::IORING_FEAT_SINGLE_MMAP
      ...
      # legacy: map the CQE ring buffer (Completion Queue Entries)
    ...
    # map the SQE ring buffer (Submission Queue Entries)
  ...
  def register(opcode : UInt32, arg : Pointer | Nil, arg_sz = 0) : Nil
    argp = arg ? arg.as(Void*) : Pointer(Void).null
    err = Crystal::System::Syscall.io_uring_register(@fd, opcode, argp, arg_sz.to_u32)
  ...
  def submit(count = 1, & : LibC::IoUringSqe* ->) : Nil
    ...
    count.times do |i|
      ...
      # set sqarray index (same index because we map 1:1 sqe and sqarray)
