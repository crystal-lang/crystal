...
class Thread
  ...
  def initialize(&@func : ->)
    ...
    if ret != 0
      raise RuntimeError.from_errno("pthread_create", ret)
    end
  end
  ...
  {% if flag?(:android) || flag?(:openbsd) %}
    ...
    @@current_key = begin
      ...
      raise RuntimeError.from_errno("pthread_key_create", ret) unless ret == 0
      current_key
    end
    ...
    protected def self.current=(thread : Thread) : Thread
      ...
      raise RuntimeError.from_errno("pthread_setspecific", ret) unless ret == 0
      thread
    end
  {% else %}
    @[ThreadLocal]
  ...
  private def stack_address : Void*
    ...
    {% elsif flag?(:freebsd) %}
      ...
      unless ret == 0
        ...
        raise RuntimeError.from_errno("pthread_attr_init", ret)
      end
      ...
      raise RuntimeError.from_errno("pthread_attr_destroy", ret) unless ret == 0
    {% elsif flag?(:linux) %}
    ...
    {% elsif flag?(:openbsd) %}
      ...
      raise RuntimeError.from_errno("pthread_stackseg_np", ret) unless ret == 0
