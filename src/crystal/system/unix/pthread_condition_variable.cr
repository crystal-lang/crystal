...
class Thread
  ...
  class ConditionVariable
    def initialize
      ...
      raise RuntimeError.from_errno("pthread_cond_init", ret) unless ret == 0
    ...
    def signal
      ...
      raise RuntimeError.from_errno("pthread_cond_signal", ret) unless ret == 0
    end

    def broadcast
      ret = LibC.pthread_cond_broadcast(self)
      raise RuntimeError.from_errno("pthread_cond_broadcast", ret) unless ret == 0
    end

    def wait(mutex : Thread::Mutex)
      ret = LibC.pthread_cond_wait(self, mutex)
      raise RuntimeError.from_errno("pthread_cond_wait", ret) unless ret == 0
    end
    ...
    def wait(mutex : Thread::Mutex, time : Time::Span)
      ...
      when Errno::ETIMEDOUT
        ...
        raise RuntimeError.from_errno("pthread_cond_timedwait", ret)
      end
    end
